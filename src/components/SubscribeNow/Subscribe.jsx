  ...
  const validateField = (name, value, isSubmit = false) => {
    switch (name) {
      ...
      case 'pickupAddress':
        ...
        if (value.trim() && value.trim().length > 200) return 'Address must be less than 200 characters';
        if (value.trim() && value.trim().length < 10) return 'Please enter a complete address';
        return '';
      
      case 'serviceInterested':
        if (isSubmit && (!value || value === '' || value === 'Select Service')) return 'Please select a service';
        return '';
      
      case 'preferredPickupDate':
        if (isSubmit && !value.trim()) return 'Please select your preferred pickup date';
        if (value.trim()) {
          ...
          if (selectedDate > thirtyDaysFromNow) return 'Please select a date within the next 30 days';
        }
        return '';
      
      case 'preferredTimeSlot':
        if (isSubmit && (!value || value === '' || value === 'Select Time Slot')) return 'Please select a preferred time slot';
        return '';
      
      case 'specialInstructions':
        if (value.length > 500) return 'Special instructions must be less than 500 characters';
        return '';
      ...
      <div className="w-full max-w-4xl">
        ...
        <div className="text-center mb-8 relative">
          ...
          <div className="relative">
            <div className="flex items-center justify-center mb-4">
              <Calendar className="text-blue-600 mr-2 animate-float" size={32} />
        ...
        {showSuccess && (
          <div className="fixed top-4 right-4 bg-green-500 text-white px-6 py-3 rounded-lg shadow-lg z-50 flex items-center animate-slide-in">
            ...
            Pickup scheduled successfully! We'll contact you within 2 hours to confirm.
          </div>
        )}
        ...
        <div className="bg-white/70 backdrop-blur-lg rounded-3xl shadow-2xl p-8 border border-white/20">
          ...
          <div className="mb-8">
            <div className="flex items-center mb-6">
              ...
              <h2 className="text-xl font-bold text-gray-800">Service Preferences</h2>
            </div>
